...
**Cari task:** **AI-2.5 gate review (operator) → G-V validasiya**. Kod tərəfi tam: adapter kontraktı + contract test · E2E 0 oversell · reconciliation · rate-limit+retry+DLQ · OTel metrik · swap-ready · audit (C1/C2/H1/H4/H5) · **H6 sync deploy-edilə-bilən (registry-driven wiring)** · **AI-2.8 Posnet connector TAM (8.1-8.3, mock-first; pull + push write-back)**. Növbəti kod-işi yoxdur (real Posnet swap interfeys/credential gözləyir — gated) — gate operator təsdiqi gözləyir.
**Son commit:** `c76b109` — feat(pos): AI-2.8.4 Posnet katalog dövri sync cron (pull entrypoint)
**Son uğurlu verify:** 2026-06-05; AI-2.8.4 TAM (Python **628 @ 98.3%**; ruff+format+mypy+bandit+detect-secrets keçir). G-V demo dövrəsi tam (admin-web). AI-2.5 gate: HUMAN-GATES.md (təsdiq gözləyir).
