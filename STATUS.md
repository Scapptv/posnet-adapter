...
**Cari task:** **Part V Faza-1 → V1.1 — 2-ci marketplace adapteri (mock-first)**. G-V CONTINUE (operator); ilk dilim ən aşağı-riskli + Docker-siz yoxlanılan: fərqli wire şəkilli 2-ci mock kanal + adapter (`ChannelAdapter`) + contract test → "yeni kanal = 1 adapter, fərqli wire, eyni engine". Plan: AI-ROADMAP Part V Faza-1 (V1.1-V1.4). Real Birmarket/Trendyol swap = D-002/Q-003 (gated, paralel). ⚠️ Build validasiya datasından ƏVVƏL gedir (operator override; validasiya kit paralel).
