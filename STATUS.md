...
**Cari task:** **Part V Faza-1 → V1.3 — real-adapter readiness** (auth variant seam, batch push partial-failure, category mapping table — non-speculative hardening + ADR). **V1.1 ✅** (`e91e18b`) Mock Bazar 2-ci adapter (eyni contract suite). **V1.2 ✅** (`2d2b887`) multi-channel fan-out — 1 məhsul → 2 kanal, sifariş → stok hər iki kanalda düşür (E2E). Real Posnet env/swap = **sonraya** (operator: "Posnet içinə daşıyanda"). V1.4 real Birmarket/Trendyol = D-002/Q-003 (gated).
