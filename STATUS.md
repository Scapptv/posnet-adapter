...
**Cari faza:** AI-2 (POS CORE) — G1 ✅ **şərti təsdiq** (2026-06-03, Scapptv); AI-1 Foundation TAM (18/18); **Faza AI-2.H TAM (H1-H5)**; **AI-2.5 IN_PROGRESS (5.1-5.5 ✅)**
**Cari task:** **AI-2.5.6** Reconciliation cron + OTel observability. Dilimlər: **5.6.1 ✅** (kanal-oxu səthi: `fetch_listing` + ADR-0019); **5.6.2 ✅** (reconcile engine: drift detect+təmir + `ChannelGuard` + `make reconcile`); **5.6.3** OTel span (`channel.push`/`channel.ingest`) + metrik (push success rate, DLQ depth, sync lag).
**Son commit:** `7989786` — feat(sync): AI-2.5.6.2 channel reconciliation engine
**Son uğurlu verify:** 2026-06-04; AI-2.5.6.2 TAM (main suite 567 @ 98.44%; guard.py 100%, reconcile.py ~100%; ruff+format+mypy+bandit+detect-secrets keçir)
...
- [ ] **AI-2.5.6** Reconciliation cron + OTel observability *(§17.4)*. Dilimlər:
  ...
  - [ ] **5.6.3** OTel — sync op span-ları (`channel.push`/`channel.ingest`) + custom Prometheus metrik (push success rate, DLQ depth via `pgmq.queue_length`, sync lag).
