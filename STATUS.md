...
**Cari task:** **Part V Faza-1 → V1.2 — multi-channel fan-out** (1 məhsul → N kanal publish; 1 kanalda sifariş → stok BÜTÜN kanallarda düşür; E2E 2 kanal). **V1.1 ✅** (`e91e18b`): Mock Bazar 2-ci marketplace adapteri eyni `AdapterContractTests` suite-i **dəyişmədən** keçir — fərqli wire şəkli (nested money, list category, PUT, fərqli vocab) adapter mapping-ində udulur. Plan: AI-ROADMAP Part V Faza-1. Real Birmarket/Trendyol = D-002/Q-003 (gated, paralel). ⚠️ V1.2 DB/dispatcher → Docker lazım (flaky); build validasiya datasından əvvəl (operator override, kit paralel).
**Son commit:** `e91e18b` — feat(channel): Part V V1.1 2-ci marketplace adapteri (Mock Bazar)
**Son uğurlu verify:** 2026-06-05; **V1.1 TAM** — Mock Bazar adapter **17 test Docker-siz yaşıl** (contract suite + mapping round-trip + error klassifikasiya; ASGI transport) + wiring 2-adapter registry + mock-marketplace regresiya yox; ruff+format+mypy(107 fayl)+bandit+detect-secrets keçir. (Əvvəlki tam suite **634 @ 98.3%** Docker stabil pəncərədə.) ⚠️ **Docker Desktop qeyri-stabil** — DB-testlər pəncərə-həssas; Docker-siz hissələr (adapter/contract/unit) həmişə yoxlanılır. **G-V demo artifact:** [test_e2e_full_loop.py](tests/integration/test_e2e_full_loop.py).
...
- [x] **AI-2.5.4** ✅ — 2026-06-04. **Webhook ingress** ([services/core/app/api/v1/webhooks.py](services/core/app/api/v1/webhooks.py)): `POST /v1/channels/{tenant_id}/{code}/webhook` (channel code per-tenant UNIQUE → tenant_id URL-də). **HMAC verify** ([libs/adapter/hmac_verify.py](libs/adapter/hmac_verify.py)): constant-time `hmac.compare_digest`, `sha256=<hex>` Stripe-style + bare hex. **Protocol extended**: `normalize_webhook(body, headers) → CanonicalOrder` (pure, sync); `AdapterCapabilities.webhook_signature_header` (per-channel HMAC header name). **MockMarketplaceAdapter** updated: capabilities.supports_webhook_orders=True, `X-Mock-Signature` header, normalize_webhook (json.loads → OrderDTO shape → `_normalise_order`). **Migration 0011** → **0012**: `channel_orders` (tenant_id, channel_id, channel_order_id UNIQUE per channel, canonical_payload JSONB, status [received/reserved/fulfilled/rejected], received_at) — RLS FORCE + posnet_app grant. **Idempotency:** `UNIQUE(channel_id, channel_order_id)` SAVEPOINT pattern — redelivered webhook → 200 OK status=duplicate (no second row). **Status codes:** 200 ok/duplicate, 401 bad/missing HMAC, 404 channel not found / inactive, 400 malformed body (AdapterPermanentError), 503 secret/factory not configured. 16 yeni test (7 HMAC unit + 9 integration: happy + idempotent + 2 HMAC failure + 2 channel lookup + secret-missing + body-malformed + factory-unwired) → suite **544** @ 83.4%. *(§17.3 inbound)*
- [x] **AI-2.5.5** ✅ — 2026-06-04. **E2E MVP** (roadmap §17.6 — crown jewel; POS↔kanal tam dövrəsi). **Inbound order ingest** ([services/core/app/sync/order_ingest.py](services/core/app/sync/order_ingest.py)): `reserve_order` (anti-oversell nüvəsi — sku→variant tenant-scoped, online-sellable anbarlar üzrə greedy allokasiya, hər rezerv `apply_movement(kind=reserve)` ilə → `SELECT FOR UPDATE` + `_effect` guard + `inventory.movement.applied` event miras alır → dispatcher push_stock hər kanala = "stok hər yerdə azalır"; bir qısa sətir bütün rezervi rollback edir — all-or-nothing SAVEPOINT) + `ingest_channel_order` (idempotent — `channel_orders` INSERT əvvəl = `UNIQUE(channel_id, channel_order_id)` guard redelivery rezervi **ikiqatlamır**; reservation SAVEPOINT-də → rejection sifarişi `rejected` saxlayır, itmir). **Webhook genişləndi** ([webhooks.py](services/core/app/api/v1/webhooks.py)): normalize → ingest → commit → best-effort `acknowledge_order` (reserved→`confirmed` / rejected→`cancelled`; `AdapterError` swallow + log, reconciliation 5.6 catch-up). **Anti-oversell bug tutuldu**: planlama oxusu `Inventory` ORM entity-ni identity-map-ə yükləyirdi → `apply_movement`-in `FOR UPDATE`-i köhnə (kilidlənməmiş) dəyəri qaytarırdı → konkurent iki sifariş hər ikisi rezerv edirdi; düzəliş = **sütun-only planlama oxusu** (entity yükləmir, kilidlənmiş oxu ilk olur). **E2E** ([tests/integration/test_e2e_mvp.py](tests/integration/test_e2e_mvp.py)): mock kanal (ASGI) + dispatcher outbound (real outbox drain) + inbound ingest/ack — POS→push_listing→mock görür→price/stock sync→webhook reserve→stok hər yerdə azalır→ack; + ikinci sifariş availability-dən artıq = **0 oversell**. 11 yeni test (10 order_ingest + 1 e2e); order_ingest **100%** coverage; webhook testləri reserved/duplicate semantikasına yeniləndi (`httpx.MockTransport` ack); main suite **555 @ 98.40%**. *(§17.6)*
- [x] **AI-2.5.6** ✅ Reconciliation cron + OTel observability *(§17.4)*. Dilimlər:
  - [x] **5.6.1** ✅ — 2026-06-04. **Kanal-oxu səthi** (ADR-0019): `ChannelAdapter.fetch_listing(sku) → ChannelListingSnapshot | None` (capability-gated `supports_fetch_listing`; push-only `NotImplementedError`) + `ChannelListingSnapshot` (sku/stock/price/currency/external_id/status) + mock `GET /listings/{sku}` ([app.py](mocks/mock_marketplace/app.py)) + `MockMarketplaceAdapter.fetch_listing` (404→None; `_request`→`_send`+`_raise_for_status` refactor). Contract suite fetch happy+404→None; Protocol stub-ları yeniləndi. 558 @ 98.38%.
  - [x] **5.6.2** ✅ — 2026-06-04. **Reconcile engine** ([sync/reconcile.py](services/core/app/sync/reconcile.py)): `reconcile_channel`/`reconcile_tenant` — hər aktiv pushed listing üçün `fetch_listing` (kanal) vs `build_canonical_product` (POS) müqayisə; stock/price drift → `push_stock`/`push_price` təmir. Read-only POS; batch skip-on-error (bir listing xətası → log+skip, run davam). **`ChannelGuard`** ([sync/guard.py](services/core/app/sync/guard.py)): dispatcher rate-limit+breaker məntiqi çıxarıldı (reuse — həm dispatcher həm reconcile); `call()` xəta qaldırır, hər çağıran öz siyasətini qoyur. `scripts/reconcile_channel_stock.py` + `make reconcile` (cron, per-tenant scope; qeydiyyatsız adapter skip). 9 test (gate: real mock injected drift→repair; price/no-drift/skip-lər/report); guard 100%, reconcile ~100%. 567 @ 98.44%.
  - [x] **5.6.3** ✅ — 2026-06-05. **OTel observability** ([sync/observability.py](services/core/app/sync/observability.py)): span helper (`channel.push` dispatcher `_guard`-da, `channel.ingest` webhook-da; qlobal tracer — OTel söndürülmüşdə no-op) + Prometheus metriklər ayrıca `SYNC_REGISTRY`-də: `posnet_sync_push_total{channel_code,operation,outcome}` (push success rate), `posnet_sync_dlq_depth{queue}` (`collect_dlq_depth` via `pgmq.queue_length`), `posnet_sync_lag_seconds{event_type}` (outbox→dispatch lag). `render_sync_metrics()` exposition. dispatcher `_guard` operation param + outcome metrik; `__call__` lag. 9 test (span monkeypatch + InMemory exporter, _guard outcome-lar, dlq gauge, render, real pgmq). 576 @ 98.41%. **Merge qeydi:** `_guard` C2 (`_SKIPPED` sentinel) ilə birləşdi — outcome metriki + swallowed-push-stale-mark birgə.
...
- [—] **AI-2.5-POS Sale/çek** — **hub scope-da DEYİL** (ADR-0021): in-store satış/çek/X-Z report = mövcud **Posnet POS-un** işidir; bu hub POS yazmır. Hub-ın "satışı" = inbound **kanal sifarişi** (AI-2.5.5 reserve → stok düş → AI-2.8.3 Posnet write-back). Mağaza POS UI Posnet-dədir.
- [x] **AI-2.6 CanonicalProduct/Inventory/Price map** ✅ — **AI-2.H5-də çatdırıldı** (2026-06-04): [sync/canonical.py](services/core/app/sync/canonical.py) — `build_canonical_product` orchestrator + `to_canonical_inventory/price/product` + `aggregate_online_stock` (ORM → canonical_model; `stock_qty=max(qty−reserved,0)`). Bax AI-2.H5 (B6).
...
- [x] **AI-2.8 Posnet connector** ✅ — 2026-06-05 (source-POS adapter; POS = ayrı Posnet, **Flutter YAZILMIR**) — ADR-0021, §17.7. Mock-first TAM (real Posnet swap = interfeys/credential gələndə, gated). **8.1 ✅** (`96d5e68`): `PosSourceAdapter` Protocol (`pull_catalog`/`push_order`) + `MockPosnetSource` (in-memory, mock-first) + `sync_catalog_from_pos` (pull → hub catalog/inventory upsert — SKU-keyed variant create/update, qiymət refresh, stok target-ə adjust; Posnet=master, idempotent); 3 test. **8.2 ✅** (`44d5ec5`): HTTP mock Posnet ([mocks/mock_posnet/app.py](mocks/mock_posnet/app.py): `GET /catalog` + `POST /orders` + `MockPosnetStore` + strict wire-modellər) + **`PosnetConnector`** ([services/core/app/adapters/posnet/connector.py](services/core/app/adapters/posnet/connector.py)): httpx `PosSourceAdapter` — `pull_catalog`→`CanonicalProduct[]`, `push_order`→`POST /orders`; HTTP xəta→AdapterError klassifikasiya marketplace adapteri ilə eyni (401/403 Auth, 429 RateLimit, 4xx Permanent, 5xx/timeout Retryable); real Posnet swap = yalnız base-URL. 10 test (ASGI transport, connector 100%) → 616 @ 98.23%. **8.3 ✅** (`284318f`): `push_order` write-back inbound axına bağlandı — webhook reserve uğurlu olduqda `_write_order_to_pos` ([webhooks.py](services/core/app/api/v1/webhooks.py)) sifarişi `pos_source.push_order(canonical)` ilə Posnet-ə yazır (best-effort, commit-sonrası, ack kimi; xəta webhook-u fail etmir / rezervi geri almır; capability-gated; POS qoşulmayanda no-op). `PosSourceFactory` + `build_pos_source_factory` ([wiring.py](services/core/app/sync/wiring.py)) `POSNET_BASE_URL`-dən qurur (boş→no-op); main.py `sync_enabled` altında `app.state.pos_source_factory` qoşur. 9 test (5 webhook write-back: reserved→push, rejected/read-only/duplicate skip, POS fail→webhook salamat; 4 wiring) → **625 @ 98.2%** (wiring.py 100%). **8.4 ✅** (`c76b109`): pull tərəfi deploy-edilə-bilən cron (§17.7 "dövri sync", push-dakı `make reconcile`-ə simmetrik) — `sync_tenant_catalog_from_pos` ([pos_ingest.py](services/core/app/sync/pos_ingest.py)) tenant-in birinci online-sellable anbarını tapır + POS stoku ora mirror edir (online anbar yoxdursa None→skip); `scripts/sync_pos_catalog.py` + `make pos-sync` (per-tenant connector `POSNET_BASE_URL`-dən, boş→no-op, apply_tenant_scope, aclose). 3 test (wrapper happy/no-warehouse/0-stock) → pos_ingest.py 100%, **628 @ 98.3%**. **Qalan (gated):** real Posnet connector — swap-ready (`de8a54d`: auth seam + ADR-0022) + **interfeys MƏLUM oldu** (2026-06-05): operator real Posnet-i göstərdi (`Posnet A`, "API keys" sessiyası) → AI araşdırdı + sənədləşdirdi **[docs/integrations/posnet-connector-spec.md](docs/integrations/posnet-connector-spec.md)**. Real Posnet = **UltimatePOS Connector API** (Passport OAuth2 password grant → Bearer; pull `GET /connector/api/variation`; push `POST /connector/api/sell`). **Q-003 gate DARALDI → yalnız credential → Vault** (base_url, oauth client_id/secret, api user/password; spec §6). Connector build (OAuth token manager + real mapping + SKU→id resolution) credential Vault-a yazılanda — interfeys artıq spekulyativ deyil.

**Follow-up (G2-yə qədər həll — AI-2.H sonrası vəziyyət):**
- AI-2.1: ~~`list_products` paginasiya~~ ✅ **2026-06-05** (`82c3a36`): `limit`/`offset` (deterministik `ORDER BY name, id`; `limit=None`→hamısı, geriyə uyumlu) + `count_products` + `X-Total-Count` header (gövdə hələ list). Qalan: `/variants/lookup` cavabına `currency`+`product_name` (post-ADR-0021 ikincil — in-store scan Posnet-də). ~~`UNIQUE(tenant_id, barcode)`~~ ✅ **H2** (migration 0010)
- AI-2.2: ~~`transfer` movement (2 warehouse atomik)~~ ✅ **2026-06-05** (`ab8534a`): `transfer_stock` (out mənbə + in hədəf, eyni tx → stok qorunur; anti-oversell; hər leg movement.applied emit) + `POST /v1/inventory/transfers`; 4 integration + 1 unit (qty-guard). ~~inventory ilk-yaranma konkurent race~~ ✅ **H2** (IntegrityError → ConflictError 409)
