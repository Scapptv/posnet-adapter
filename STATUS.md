...
**Cari task:** **AI-2.5.6** Reconciliation cron + OTel observability. Dilimlər: **5.6.1 ✅** (kanal-oxu səthi: `fetch_listing` + ADR-0019); **5.6.2** reconcile engine (drift detect+təmir + cron); **5.6.3** OTel span + metrik (push success rate, DLQ depth, sync lag).
**Son commit:** `8a4ac18` — feat(adapter): AI-2.5.6.1 channel fetch_listing read surface
**Son uğurlu verify:** 2026-06-04; AI-2.5.6.1 TAM (main suite 558 @ 98.38%; ruff+format+mypy+bandit+detect-secrets keçir)
...
- [ ] **AI-2.5.6** Reconciliation cron + OTel observability *(§17.4)*. Dilimlər:
  ...
  - [ ] **5.6.2** Reconcile engine — `fetch_listing` (kanal) vs `build_canonical_product` (POS) drift → `push_stock` təmir; `scripts/reconcile_channel_stock.py` + `make reconcile`; injected-drift test (gate). Per-channel rate-limit/breaker reuse.
