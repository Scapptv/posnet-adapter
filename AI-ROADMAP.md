...
| 6 | POS | **Posnet** (mövcud ERP — ayrı layihə); hub Posnet-ə connector ilə qoşulur, POS/kassir UI **yazılmır** |
...
| POS | **Posnet** (mövcud ERP, ayrı layihə) | — |
| Frontend | TypeScript + React (admin-web — hub merchant paneli) | — |
...
│
├── apps/  (admin-web = hub merchant paneli · storefront ❄️) — POS = ayrı Posnet layihəsi, burada yazılmır
...
- AI-2.7 Admin-web minimal: hub merchant paneli — məhsul/stok idarəsi + publish + sync görünüşü (React) ✅
- AI-2.8 **Posnet connector** (source-POS adapter): Posnet ERP-dən məhsul/stok/qiymət pull + sifariş/çek push (canonical+adapter, mock-first → real). **Flutter POS yazılmır** — POS = ayrı Posnet layihəsi. Bax §17.7.
...
### 17.7 POS-tərəfi: Posnet connector (source-POS adapter)

**POS = mövcud Posnet** (ayrı layihə, real ERP — `tək həqiqət mənbəyi`). Bu hub **POS YAZMIR**;
Posnet-in datasını alıb kanallara çıxarır + sifarişləri geri yazır. Bağlantı = **Posnet connector**,
kanal adapterləri ilə **eyni nümunə** (canonical model + adapter Protocol), sadəcə mənbə kanal yox POS-dur:
- **Pull (Posnet → hub):** məhsul/variant/stok/qiymət → canonical → hub-ın online catalog/inventory
  (curated online subset). Mexanizm: Posnet API / webhook / dövri sync.
- **Push (hub → Posnet):** kanal sifarişi/çek/stok-azalma → canonical → **Posnet-ə yaz** (inbound axının
  davamı — webhook→reserve→`acknowledge` artıq var; son addım Posnet-ə yazmaq).

**Mock-first → real** (marketplace adapteri kimi): real Posnet API/credential gələnə qədər hub-ın
catalog/inventory-si admin-web/API ilə doldurulur (G-V demo üçün yetərli). Posnet interfeysi
(API/DB/format + auth) müəyyən olunanda connector yazılır (**AI-2.8**).

**Flutter kassir / `apps/pos-flutter` yoxdur** — POS UI Posnet-dədir.
...
- [x] **V1.1 — 2-ci marketplace adapteri (mock-first)** ✅ — 2026-06-05 (`e91e18b`). `mocks/mock_bazar` (fərqli wire şəkli: nested `price`/`totals`, list category, PUT verb, `/products`+`/sales`, `live`/`new` vocab) + `MockBazarAdapter` (`ChannelAdapter`; shape↔canonical + vocab translation) + `register_builtin_adapters` 2 adapter qeyd edir. **Eyni `AdapterContractTests` suite-i dəyişmədən keçir** → tezis sübut. 17 test Docker-siz (ASGI).
- [x] **V1.2 — Multi-channel fan-out** ✅ — 2026-06-05 (`2d2b887`). E2E ([test_e2e_multichannel.py](tests/integration/test_e2e_multichannel.py)): 1 məhsul → 2 fərqli-şəkilli kanal (mock-marketplace + mock-bazar) publish; 1 kanalda sifariş → stok **BÜTÜN** kanallarda düşür — dispatcher fan-out + 2 registered adapter, **per-channel engine kodu yox**. Bazar adapter parite (normalize_webhook/network/from_channel) → 100%. Tam suite **655 @ 98.0%**.
